"""
BFMC Hybrid Pilot - Version 3.1 (Complete Scene Integration)
Includes:
- Original Hybrid Lane Tracking & Pure Pursuit
- Complete BFMC Scene Analyzer Integration
- Strict Traffic Light Spatial Filtering (Ignores fake red objects)
"""
...
# ---------------------------------------------------------------------------
# Scene Analyzer Import
# ---------------------------------------------------------------------------
try:
    from bfmc_complete_detector import BFMCCompleteDetector
    _HAS_SCENE_ANALYZER = True
except ImportError:
    print("WARNING: bfmc_complete_detector.py not found in the same folder.")
...
# ===========================================================================
# TRAFFIC DECISION MAKER (Strict Filtering)
# ===========================================================================
class TrafficDecisionMaker:
    """Evaluates the scene analyzer results and dictates speed limits."""
    def __init__(self):
        self.stop_sign_timer = 0
        self.cleared_stop_sign = False

    def evaluate(self, base_speed, scene_results):
        if not scene_results:
            return base_speed, ""

        # 1. Immediate Hazards (Pedestrians & Vehicles)
        if len(scene_results.get('pedestrians', [])) > 0:
            return 0.0, "YIELD: PEDESTRIAN"
            
        if len(scene_results.get('vehicles', [])) > 0:
            return 0.0, "YIELD: VEHICLE AHEAD"

        # 2. Traffic Lights (Strict Bounding Box Filtering)
        for tl in scene_results.get('traffic_lights', []):
            ...
            if 'RED' in str(val).upper():
                ...
                if bbox is not None:
                    ...
                    # RULE 1: Position (Must be in top half of image)
                    if y > 240:
                        continue
                    
                    # RULE 2: Shape (Must be vertical or square-ish)
                ...
                return 0.0, "STOP: RED LIGHT"

        # 3. Stop Signs (Requires 3-second halt)
        is_stop_sign = False
...
class BFMC_Pilot:
    ...
    def run(self):
        print("BFMC Pilot v3.1: STARTING — INTEGRATED PERCEPTION & DECISION MAKER")
        try:
            while True:
                ...
                if self.cam_ok:
                    ...
                    frame = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR) # RGB to BGR Fix
                else:
                    frame = np.zeros((480, 640, 3), dtype=np.uint8)

                # --- 1. NEW: Comprehensive Scene Analysis ---
                scene_results = None
                ...
                # --- 2. NEW: Traffic Decision Maker ---
                decision_speed, decision_reason = self.decision_maker.evaluate(base_speed, scene_results)

                # --- 3. Image Processing (BEV) ---
                ...
                raw_steer_guarded, guard_spd, guard_on = self.guard.apply(
                    steer_angle, guard_left, guard_right, y_eval=y_eval)
                ...
                # --- 10. SPEED POLICY (Decision Maker > Curvature) ---
                ...
                # Draw Decision Override
                if decision_reason != "":
                    color = (0, 0, 255) if speed == 0 else (0, 255, 255)
                    cv2.putText(dbg, decision_reason, (150, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 3)

                if guard_on:
                    cv2.putText(dbg, "! GUARD !", (230, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
                if lost:
                    grace_label = (f"LOST ({self.lost_frames}/{LOST_GRACE_FRAMES})"
                                   if self.lost_frames <= LOST_GRACE_FRAMES else "LOST - STOPPED")
                    cv2.putText(dbg, grace_label, (130, 140), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
                ...
                if self.scene_analyzer:
                    cv2.imshow("BFMC_v3_SCENE", viz_frame)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="BFMC Pilot v3")
